...
function GestionUsuarios({ myProfileId }: { myProfileId: string }) {
  ...
  const handleCambiarRol = async (u: Profile) => {
    // Dentro del grupo agencia, alterna entre owner_agencia y employee
    const nuevoRol = u.rol === 'owner_agencia' ? 'employee' : 'owner_agencia';
        ...
        <View style={{ paddingHorizontal: 18, paddingBottom: 18 }}>
          ...
          {orgUsers.length === 0 ? (
            ...
            orgUsers.map((u) => {
              ...
              const isOwnerAgencia = u.rol === 'owner_agencia';
              const busy = loadingId === u.id;
              const label = ROL_LABEL[u.rol] ?? u.rol;
              const color = isOwnerAgencia
...
function InvitarUsuario() {
  ...
  const [rol, setRol]           = useState<'owner_agencia' | 'employee'>('employee');
  ...
  const handleInvitar = async () => {
    ...
    setEmail(''); setNombre(''); setRol('owner_agencia');
  };
      ...
      {expanded && (
        <View style={{ paddingHorizontal: 18, paddingBottom: 18, gap: 12 }}>
          ...
          <View>
            ...
            <View style={{ flexDirection: 'row', gap: 8 }}>
              {(['employee', 'owner_agencia'] as const).map((r) => (
                <TouchableOpacity key={r} onPress={() => setRol(r)} style={{
                  ...
                  <Text style={{ fontSize: 13, fontWeight: '600', color: rol === r ? '#2563eb' : '#94a3b8' }}>
                    {r === 'owner_agencia' ? 'Dueño Agencia' : 'Empleado'}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
...
export default function PerfilScreen() {
  ...
  const isOwnerAgencia = profile?.rol === 'owner_agencia';
  const isSuperOwner   = profile?.rol === 'owner';
  const canManageUsers = isOwnerAgencia || isSuperOwner;
    ...
    <ScrollView style={{ flex: 1, backgroundColor: '#f8fafc' }} contentContainerStyle={{ padding: 20 }}>
      ...
      {isSuperOwner && (
        <View style={{ flexDirection: 'row', gap: 10, marginBottom: 16 }}>
          <TouchableOpacity
            onPress={() => router.replace('/negocio')}
            style={{
              flex: 1, backgroundColor: '#f0fdf4', borderRadius: 12, padding: 14,
              flexDirection: 'row', alignItems: 'center', gap: 8,
            }}
          >
            <Text style={{ fontSize: 16 }}>↪️</Text>
            <Text style={{ color: '#16a34a', fontWeight: '600', fontSize: 14 }}>Ir a Negocio</Text>
          </TouchableOpacity>
          <TouchableOpacity
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
          ...
          <View>
            ...
            <View style={{
              ...
              <Text style={{ fontSize: 11, fontWeight: '600', color: '#2563eb' }}>{rolLabel}</Text>
            </View>
          </View>
        </View>
